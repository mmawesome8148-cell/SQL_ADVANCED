...
### 1. SELECT 주요 절
`select_expr`: 조회할 컬럼 또는 표현식 지정
- select *: 모든 컬럼 선택
- select tbl_name: 특정 테이블의 모든 컬럼 선택

`FROM table_references`: 조회할 테이블 지정
- 여러 테이블을 지정하면 조인
- 테이블, 컬럼 별칭 가능

`PARTITION`: 특정 파티션만 조회 가능

`WHERE`: 행 필터링 조건, 집계 함수 사용 불가
...
FROM (SELECT region
            ...
            ,ROW_NUMBER() OVER (PARTITION BY region 
                                 ORDER BY review_count DESC) AS rn
